...
if (isset($_POST['add_event'])) {
    ...
    if (!empty($event_name)) {
        ...
        $check_event_query = "SELECT * FROM events WHERE event_name = '$event_name'";
...
if (isset($_POST['remove_event'])) {
    // Check if an event_id is passed and is valid
    if (isset($_POST['event_id']) && !empty($_POST['event_id'])) {
        ...
        try {
            // 1. Prepare and delete candidates first
            ...
            mysqli_stmt_bind_param($stmt_candidates, "i", $event_id); // "i" for integer
            ...
            mysqli_stmt_bind_param($stmt_event, "i", $event_id); // "i" for integer
        ...
        // Close prepared statements only if they were initialized
        if (isset($stmt_event)) {
    ...
    // Redirect to avoid re-submission and show message after reload
    header("Location: " . $_SERVER['PHP_SELF']);
    exit();
}
...
$vote_count_query = "SELECT COUNT(*) AS total_votes FROM votes";
